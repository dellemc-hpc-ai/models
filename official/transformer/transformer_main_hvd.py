...
from official.transformer.utils import dataset_hvd # added for horovod
...
def record_scalars(metric_dict):
  ...
  for key, value in metric_dict.items():
    # changed from tf.contrib.summary.scalar to tf.summary.scalar
...
def get_learning_rate_2(learning_rate, learning_rate_warmup_steps, warmup_init_lr):
  with tf.name_scope("learning_rate"):    

    warmup_steps = float(learning_rate_warmup_steps)
    steps = tf.to_float(tf.train.get_or_create_global_step())
    
    warmup_end_lr=learning_rate
    
    lr_step = (warmup_end_lr - warmup_init_lr) / learning_rate_warmup_steps
    decay_factor = warmup_end_lr * learning_rate_warmup_steps ** 0.5
    #lr=warmup_init_lr
    
    learning_rate = tf.cond(steps < learning_rate_warmup_steps, lambda: (warmup_init_lr + steps * lr_step), lambda: (decay_factor * steps ** - 0.5))
...
def get_train_op_and_metrics(loss, params):
  ...
  with tf.variable_scope("get_train_op"):
    # added to add include  different  learning rate scheme
    if params["lr_scheme"]==0:
      # scheme 0 has constant learning rate
      with tf.name_scope("learning_rate"):
        learning_rate = tf.identity(params["learning_rate"], "learning_rate")

    elif params["lr_scheme"]==1:
      learning_rate = get_learning_rate(
          learning_rate=params["learning_rate"],
          hidden_size=params["hidden_size"],
          learning_rate_warmup_steps=params["learning_rate_warmup_steps"])
    elif params["lr_scheme"]==2:
      learning_rate = get_learning_rate_2(
          learning_rate=params["learning_rate"],
          learning_rate_warmup_steps=params["learning_rate_warmup_steps"],
          warmup_init_lr=params["warmup_init_lr"])

    if params["opt_alg"]=="lazyadam":
    ...
    elif params["opt_alg"]=="sgd_momentum":
        # creates SGD with momentum optimizer
        optimizer = tf.train.MomentumOptimizer(
            learning_rate * hvd.size(),
            momentum=params["optimizer_sgd_momentum"])

    elif params["opt_alg"]=="rmsprop":
        # creates RMSprop optimizer
        optimizer = tf.train.RMSPropOptimizer(
            learning_rate * hvd.size(),
            decay=params["optimizer_rms_decay"],
            momentum=params["optimizer_rms_momentum"],
            epsilon=parmas["optimizer_rms_epsilon"])

    if params["use_tpu"] and params["tpu"] != tpu_util.LOCAL:
      optimizer = tf.contrib.tpu.CrossShardOptimizer(optimizer)

    # Horovod: add Horovod Distributed Optimizer.
    # added sparse_as_dense option
    ...
    # Made colocate_gradients_with_ops=False
    gradients = optimizer.compute_gradients(
        loss, tvars, colocate_gradients_with_ops=False)
...
def run_loop(
  ...
  **Step vs. Epoch vs. Iteration**
  ...
  for i in xrange(schedule_manager.train_eval_iterations):
    ...
    estimator.train(
       dataset_hvd.train_input_fn,
        steps=schedule_manager.single_iteration_train_steps,
        hooks=train_hooks)

    eval_results = estimator.evaluate(
        input_fn=dataset_hvd.eval_input_fn,
...
def construct_estimator(flags_obj, params, schedule_manager):
  ...
  # Return statement modified for horovod
  if not params["use_tpu"]:
    # removed distribution strategy below including the return statements    

    #distribution_strategy = distribution_utils.get_distribution_strategy(
    #    flags_core.get_num_gpus(flags_obj), flags_obj.all_reduce_alg)
    return tf.estimator.Estimator(
        model_fn=model_fn, model_dir=flags_obj.model_dir if hvd.rank() == 0 else None, params=params,
        config=tf.estimator.RunConfig(log_step_count_steps=flags_obj.log_step_count_steps,
                               save_checkpoints_secs=flags_obj.save_checkpoints_secs if hvd.rank() == 0 else None, 
                               session_config=tf.ConfigProto(intra_op_parallelism_threads=flags_obj.intra_op, inter_op_parallelism_threads=flags_obj.inter_op),keep_checkpoint_max=500))
...
if __name__ == "__main__":
  ...
  # added  to control the log verbosity on non rank 0 nodes
  hvd.init()
  if hvd.rank() == 0:
